...
{
    public partial class Form_Add_Ingredient_Purchases : DevExpress.XtraEditors.XtraForm
    {
        ...
        public Form_Add_Ingredient_Purchases()
        {
            InitializeComponent();
            // This line of code is generated by Data Source Configuration Wizard
            // Fill the SqlDataSource asynchronously
            sqlDataSourceIngredientItems.FillAsync();
            // This line of code is generated by Data Source Configuration Wizard
            // Fill the SqlDataSource asynchronously
        ...
        private void Form_Add_Ingredient_Purchases_Load(object sender, EventArgs e)
        {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            {
                int ingredient_food_purchase_id = ingredient_purchase.add_ingredient_purchase(Convert.ToInt32(txtBranch.EditValue), Convert.ToInt32(fieldNameSupplier.EditValue), Convert.ToInt32(txtPaymentType.EditValue), "Supplier name", 1, DateTime.Now, txtDesc.Text, txtPaymentType.Text, Convert.ToDecimal(txtFinalPrice.Text), Convert.ToDecimal(txtFinalPrice.Text), 0, DateTime.Now, DateTime.Now);

                temp_ingredient_purchase.add_temp_ingredient_purchase(Convert.ToInt32(txtBranch.EditValue), Convert.ToInt32(fieldNameSupplier.EditValue), Convert.ToInt32(txtPaymentType.EditValue), "Supplier name", 1, DateTime.Now, txtDesc.Text, txtPaymentType.Text, Convert.ToDecimal(txtFinalPrice.Text), Convert.ToDecimal(txtFinalPrice.Text), 0, DateTime.Now, DateTime.Now);

                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ingredient_purchase.add_ingredient_purchase_history(Convert.ToInt32(txtBranch.EditValue), ingredient_food_purchase_id, Convert.ToInt32(dt.Rows[i]["Food ID"]), dt.Rows[i]["Product Name"].ToString(), Convert.ToInt32(dt.Rows[i]["Product Qty"]), 10, DateTime.Now, DateTime.Now);
                    temp_ingredient_purchase.add_temp_ingredient_purchase_history(Convert.ToInt32(txtBranch.EditValue), ingredient_food_purchase_id, Convert.ToInt32(dt.Rows[i]["Food ID"]), dt.Rows[i]["Product Name"].ToString(), Convert.ToInt32(dt.Rows[i]["Product Qty"]), Convert.ToDecimal(dt.Rows[i]["Sale Price"]), 0, DateTime.Now, DateTime.Now);
                }
        ...
        private void txtProducts_EditValueChanged(object sender, EventArgs e)
        {
            ...
            {
                DataTable data = ingredient_item.get_ingredient_item(Convert.ToInt32(txtProducts.EditValue.ToString()));
        ...
        private void txtDiscount_EditValueChanged(object sender, EventArgs e)
        {
            if (txtDiscount.EditValue != null)
            {
                ...
                {
                    MessageBox.Show("من فضلك ادخل  الخصم وصحيح");
                    txtDiscount.Text = "0";
